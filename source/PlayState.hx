...
class PlayState extends MusicBeatState
{
	...
	override public function update(elapsed:Float)
	{
		...
		callHScript(CALLBACK, 'curStep', curStep);
		callHScript(CALLBACK, 'curBeat', curBeat);
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		...
		callHScript(FUNCTION, 'doDeathCheck', [skipHealthCheck]);
	}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		...
		callHScript(FUNCTION, 'triggerEventNote', [eventName, value1, value2]);
	}
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if(!ClientPrefs.getGameplaySetting('practice', false) && !ClientPrefs.getGameplaySetting('botplay', false)) {
						...
						Paths.save.data.weekCompleted = StoryMenuState.weekCompleted;
						Paths.save.flush();
					}
	...
	private function onKeyPress(event:KeyboardEvent):Void
	{
		...
		if (!cpuControlled && startedCountdown && !paused && key > -1 && (FlxG.keys.checkStatus(eventKey, JUST_PRESSED) || ClientPrefs.controllerMode))
		{
			...
			callHScript(FUNCTION, 'onKeyPress', [event]);
		}
	...
	private function onKeyRelease(event:KeyboardEvent):Void
	{
		...
		if(!cpuControlled && startedCountdown && !paused && key > -1)
		{
			...
			callHScript(FUNCTION, 'onKeyRelease', [event]);
		}
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		callHScript(FUNCTION, 'noteMiss', [daNote]);
	}
	...
	function noteMissPress(direction:Int = 1):Void //You pressed a key when there was no notes to press for this key
	{
		...
		callHScript(FUNCTION, 'noteMissPress', [direction]);
	}
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		callHScript(FUNCTION, 'opponentNoteHit', [note]);
	}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			callHScript(FUNCTION, 'goodNoteHit', [note]);
		}
	}
	...
	override function sectionHit()
	{
		...
		callHScript(FUNCTION, 'sectionHit', []);
	}
	...
	function callHScript(type:HScriptCallType, name:String, func:Dynamic)
	{
		#if hscript
		hscript.call(type, name, func);
		#end
	}
	...
	function getRatings()
	{
		...
		{
			var rating:Array<String> = ratings[i].split('||');
			ratingStuff.push([rating[0], Std.parseFloat(rating[1])]);
		}
	}
}
