...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		#if TITLE_SCREEN_EASTER_EGG
		if (Paths.save.data.psychDevsEasterEgg == null) Paths.save.data.psychDevsEasterEgg = ''; //Crash prevention
		switch(Paths.save.data.psychDevsEasterEgg.toUpperCase())
		{
			case 'SHADOW':
				titleJSON.gfx += 210;
				titleJSON.gfy += 40;
		...
		if(!initialized)
		{
			if(Paths.save.data.fullscreen)
			{
				FlxG.fullscreen = Paths.save.data.fullscreen;
		...
		if (Paths.save.data.weekCompleted != null)
		{
			StoryMenuState.weekCompleted = Paths.save.data.weekCompleted;
		}
		...
		#else
		if(Paths.save.data.flashing == null && !FlashingState.leftState) {
	...
	function startIntro()
	{
		...
		var easterEgg:String = Paths.save.data.psychDevsEasterEgg;
			...
			else if (FlxG.keys.firstJustPressed() != FlxKey.NONE)
			{
				...
				if(allowedKeys.contains(keyName)) {
					...
					for (wordRaw in easterEggKeys)
					{
						...
						{
							//trace('YOOO! ' + word);
							if (Paths.save.data.psychDevsEasterEgg == word)
								Paths.save.data.psychDevsEasterEgg = '';
							else
								Paths.save.data.psychDevsEasterEgg = word;
							Paths.save.flush();
